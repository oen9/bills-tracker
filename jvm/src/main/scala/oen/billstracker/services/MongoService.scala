...
trait MongoService[F[_]] {
  ...
  def updateGroupName(dbUser: DbUser, groupId: BSONObjectID, name: String): F[Option[UpdateWriteResult]]
  def deleteGroup(dbUser: DbUser, groupId: BSONObjectID): F[Option[UpdateWriteResult]]
}
